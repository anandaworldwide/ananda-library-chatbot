...
def signal_handler(sig: int, frame: Any) -> None:
    ...
    with _shutdown_lock:
        if _is_exiting:
            ...
            logger.info("Graceful shutdown initiated. Processing will stop soon. Press Ctrl+C again for forced exit.")
            _is_exiting = True
...
class ProgressTracker:
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        try:
            ...
            if exc_type is not None:
                ...
                logger.warning(f"Progress tracking interrupted due to exception: {exc_val}")
            elif is_exiting():
                self.state.interrupted = True
                logger.info("Progress tracking interrupted due to graceful shutdown")
            
            # Save final checkpoint if needed
            if self.checkpoint_callback and self.state.current > self.state.last_checkpoint:
    ...
    def update(self, n: int = 1, **kwargs) -> bool:
        ...
        if (self.checkpoint_callback and 
            self.state.current - self.state.last_checkpoint >= self.config.checkpoint_interval):
    ...
    def _save_checkpoint(self, checkpoint_type: str, additional_data: dict[str, Any] | None = None) -> None:
        ...
        try:
            ...
            logger.debug(f"Checkpoint saved: {checkpoint_type} at {self.state.current}")
...
class AsyncProgressTracker(ProgressTracker):
    ...
    async def save_checkpoint_async(self, checkpoint_type: str, additional_data: dict[str, Any] | None = None) -> None:
        ...
        try:
            ...
            logger.debug(f"Async checkpoint saved: {checkpoint_type} at {self.state.current}")
