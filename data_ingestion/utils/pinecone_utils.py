...
def create_pinecone_index_if_not_exists(
    ...
    if dry_run:
        confirm = input(f"Dry run: Index '{index_name}' does not exist. Create it? (Y/n): ")
    ...
    try:
        ...
        if wait_for_ready:
            print(f"Waiting for index '{index_name}' to be ready (timeout: {timeout}s)...")
...
def clear_library_vectors(
    ...
    if ask_confirmation:
        confirm = input(f"Delete ALL {len(vector_ids)} vectors for library '{library_name}'? This cannot be undone. (y/N): ")
        if confirm.lower() != 'y':
    ...
    try:
        ...
        print(f"Successfully deleted {total_deleted} vectors for library '{library_name}'")
        return True
    ...
    try:
        ...
        while True:
            ...
            if hasattr(returned_from_list, '__next__'):  # Generator
                    ...
                    while True:
                        ...
                        if isinstance(page_response, list) and page_response:
                            vector_ids_to_delete.extend([
                                ...
                                for vector in page_response if vector
                            ])
                ...
                vector_ids_to_delete = [
                    vector.id if hasattr(vector, 'id') else vector 
                    for vector in returned_from_list if vector
                ]
                ...
                if hasattr(response, 'vectors') and response.vectors:
                    vector_ids_to_delete = [
                        vector.id if hasattr(vector, 'id') else vector 
                        for vector in response.vectors if vector
                    ]
