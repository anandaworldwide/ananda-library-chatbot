...
def load_estimates():
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            with open(ESTIMATES_FILE, "a+") as f:
                ...
                try:
                    ...
                    if not content.strip():  # Check if file is empty
                        logger.info(
                            "Estimates file is empty or doesn't exist. Creating with default values."
                        )
                        default_estimates = {
            ...
            logger.error(
                f"Attempt {attempt + 1}: Error decoding JSON from {ESTIMATES_FILE}: {str(e)}"
            )
        except OSError as e:
            logger.error(
                f"Attempt {attempt + 1}: IO Error accessing {ESTIMATES_FILE}: {str(e)}"
            )
        except Exception as e:
            logger.error(
                f"Attempt {attempt + 1}: Unexpected error accessing {ESTIMATES_FILE}: {str(e)}"
            )
    ...
    logger.warning(
        f"Failed to load estimates after {MAX_RETRIES} attempts. Returning default values."
    )
    return {
        "audio_file": {"time": None, "size": None},
        "youtube_video": {"time": None, "size": None},
...
def estimate_total_processing_time(items):
    ...
    for item in items:
        if item["status"] != "completed" and item["type"] in [
            "audio_file",
            "youtube_video",
        ]:
            ...
            if (
                estimate
                and estimate["time"] is not None
                and estimate["size"] is not None
            ):
