...
class TestGetPineconeClient:
    ...
    def test_get_pinecone_client_success(self):
        ...
        with patch.dict(os.environ, {"PINECONE_API_KEY": "test-api-key"}):
            with patch('data_ingestion.utils.pinecone_utils.Pinecone') as mock_pinecone:
                mock_client = Mock()
...
class TestGetPineconeIngestIndexName:
    ...
    def test_get_index_name_missing(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError, match="PINECONE_INGEST_INDEX_NAME environment variable not set"):
                get_pinecone_ingest_index_name()
    
    def test_get_index_name_empty(self):
        """Test error when index name is empty."""
        with patch.dict(os.environ, {"PINECONE_INGEST_INDEX_NAME": ""}):
            with pytest.raises(ValueError, match="PINECONE_INGEST_INDEX_NAME environment variable not set"):
                get_pinecone_ingest_index_name()
...
class TestCreatePineconeIndexSync:
    ...
    def test_create_index_not_exists_success(self, capsys):
        ...
        mock_pinecone = Mock()
        mock_pinecone.describe_index.side_effect = [NotFoundException("Not found"), Mock(status={'ready': True})]
        ...
        with patch.dict(os.environ, env_vars):
            with patch('data_ingestion.utils.pinecone_utils.ServerlessSpec') as mock_spec:
                mock_spec_instance = Mock()
                mock_spec.return_value = mock_spec_instance
                
                create_pinecone_index_if_not_exists(mock_pinecone, "test-index", wait_for_ready=True)
    ...
    def test_create_index_dry_run_decline(self):
        ...
        with patch('builtins.input', return_value='n'):
            with pytest.raises(SystemExit, match="1"):
                create_pinecone_index_if_not_exists(mock_pinecone, "test-index", dry_run=True)
    ...
    def test_create_index_dry_run_accept(self, capsys):
        ...
        with patch.dict(os.environ, env_vars):
            with patch('builtins.input', return_value='y'):
                with patch('data_ingestion.utils.pinecone_utils.ServerlessSpec'):
                    create_pinecone_index_if_not_exists(mock_pinecone, "test-index", dry_run=True)
...
class TestClearLibraryVectors:
    ...
    def test_clear_vectors_no_vectors_found(self, capsys):
        ...
        result = clear_library_vectors(mock_index, "test-library", ask_confirmation=False)
    ...
    def test_clear_vectors_success(self, capsys):
        ...
        assert result is True
        mock_index.list.assert_called_once_with(prefix="text||test-library||", limit=100)
    ...
    def test_clear_vectors_user_confirmation_decline(self):
        ...
        with patch('builtins.input', return_value='n'):
            result = clear_library_vectors(mock_index, "test-library", ask_confirmation=True)
...
class TestClearLibraryVectorsAsync:
    ...
    async def test_clear_vectors_async_success(self, capsys):
        ...
        with patch('asyncio.to_thread') as mock_to_thread:
            ...
            mock_list_response.__next__ = Mock(side_effect=[["vec1", "vec2"], StopIteration])
            mock_to_thread.side_effect = [
...
class TestIntegration:
    ...
    def test_full_pinecone_setup_workflow(self, capsys):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            with patch('data_ingestion.utils.pinecone_utils.Pinecone') as mock_pinecone_class:
                mock_client = Mock()
