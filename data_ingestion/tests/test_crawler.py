...
from crawler.website_crawler import (WebsiteCrawler, load_config,
                                     ensure_scheme, handle_exit,
                                     sanitize_for_id)
...
class TestSQLiteIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock_sqlite_connect.side_effect = lambda db_path_arg: self.original_sqlite_connect(':memory:')
    ...
    def test_database_initialization(self):
        ...
        required_columns = {
            'url', 'last_crawl', 'next_crawl', 'crawl_frequency',
            'content_hash', 'last_error', 'status', 'retry_count',
            'retry_after', 'failure_type'
        }
        self.assertTrue(required_columns.issubset(columns), f"Missing columns: {required_columns - columns}")
        crawler.close()
    ...
    def test_seed_initial_url(self):
        ...
        expected_start_url = crawler.normalize_url(ensure_scheme(self.site_config["domain"]))
    ...
    def test_url_operations(self):
        ...
        crawler.mark_url_status(test_urls[1], 'failed', error_msg="Temporary connection error")
...
class TestChangeDetection(unittest.TestCase):
    ...
    def test_content_change_detection(self):
        ...
        crawler.cursor.execute(
            ...
            (normalized_test_url, 'visited', initial_hash, crawler.crawl_frequency_days)
        )
        crawler.conn.commit()
        
        # Test 1: Same hash should not need processing
        self.assertFalse(crawler.should_process_content(normalized_test_url, initial_hash))
...
class TestFailureHandling(unittest.TestCase):
    ...
    def test_temporary_failure_retry(self):
        ...
        cursor = crawler.conn.cursor()
        cursor.execute("SELECT status, failure_type, retry_count, retry_after FROM crawl_queue WHERE url = ?", 
                      (normalized_test_url,))
        row = cursor.fetchone()
    ...
    def test_permanent_failure(self):
        ...
        cursor = crawler.conn.cursor()
        cursor.execute("SELECT status, failure_type, retry_count FROM crawl_queue WHERE url = ?", 
                      (normalized_test_url,))
        row = cursor.fetchone()
        self.assertIsNotNone(row, "URL should exist in DB")
        self.assertEqual(row['status'], 'failed')
        self.assertEqual(row['failure_type'], 'permanent')
        self.assertEqual(row['retry_count'], 0) # Retry count should be reset for permanent
        crawler.close()
    ...
    def test_retry_failed_urls(self):
        ...
        cursor = crawler.conn.cursor()
        cursor.execute("SELECT status, retry_after, failure_type, retry_count FROM crawl_queue WHERE url = ?", (norm_perm_url,))
        ...
        cursor.execute("SELECT status, retry_after, failure_type, retry_count FROM crawl_queue WHERE url = ?", (norm_temp_url,))
...
class TestDaemonBehavior(unittest.TestCase):
    ...
    @patch('time.sleep')
    @patch('crawler.website_crawler.sync_playwright') # Mock playwright to prevent actual browser launch
    def test_daemon_sleeps_when_no_urls(self, mock_sync_playwright, mock_time_sleep):
        ...
        with patch.object(crawler, 'get_next_url_to_crawl', return_value=None):
            ...
            # Mock handle_exit.exit_requested to be False for a few iterations
            with patch('crawler.website_crawler.handle_exit') as mock_handle_exit_module:
                effect_count = 0
                ...
                type(mock_handle_exit_module).exit_requested = PropertyMock(side_effect=exit_requested_side_effect)
