...
- **ignore**: Skip this document (case-insensitive) - excludes from datasets
...
*Scoring: 0=Irrelevant, 1=Marginally Relevant, 2=Relevant, 3=Highly Relevant, ignore=Skip*
**Relevance Score** [Enter 0-3 or ignore]: 
...
class MarkdownGenerator:
    ...
    def _load_environment(self):
        ...
        try:
            ...
            print(
                f"Loaded environment for site: {self.site_id} from .env.{self.site_id}"
            )
    ...
    def get_embedding(self, text: str) -> list[float]:
        ...
        try:
            response = self.openai_client.embeddings.create(
                model="text-embedding-3-large", dimensions=3072, input=text
            )
    ...
    def retrieve_documents(
        self, query: str, top_k: int = DEFAULT_RETRIEVAL_COUNT
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if self.enabled_media_types:
                filter_parts.append({"type": {"$in": self.enabled_media_types}})

            if self.included_libraries:
                filter_parts.append({"library": {"$in": self.included_libraries}})
            ...
            for match in results.matches:
                filtered_metadata = {
                    ...
                    if k != "text"
                    and not k.startswith("loc.")
                    and not k.startswith("pdf.")
                    and k != "full_info"
                }
    ...
    def create_markdown_file(self, query: str, documents: list[dict[str, Any]]) -> str:
        ...
        for i, doc in enumerate(documents, 1):
            ...
            doc_text = DOCUMENT_TEMPLATE.format(index=i, content=content_with_metadata)
    ...
    def _load_site_config(self):
        ...
        try:
            ...
            if self.site_id not in config:
                print(
                    f"Warning: Site ID '{self.site_id}' not found in config.json. Using default filtering."
                )
                return {}
    ...
    def _get_included_libraries(self):
        ...
        if not self.site_config:
            print(
                "Warning: No site configuration found. No library filtering will be applied."
            )
            return []
        ...
        if not libraries:
            print(
                "Warning: No included libraries found in site config. No library filtering will be applied."
            )
    ...
    def run_diagnostics(self):
        ...
        try:
            ...
            results = self.index.query(
                vector=[0.1] * index_stats.dimension, top_k=100, include_metadata=True
            )
            ...
            if not results.matches:
                ...
                if self.included_libraries and self.enabled_media_types:
                    ...
                    if not test_results.matches:
                        print(
                            "WARNING: No documents match your libraries AND media types filters!"
                        )
                        print(f"Available libraries: {libraries}")
                        print(f"Available media types: {media_types}")
                    else:
                        print(
                            f"Found {len(test_results.matches)} documents matching your filters."
                        )
        ...
        try:
            ...
            for i, question in enumerate(questions, 1):
                ...
                if documents:
                    ...
                    print(
                        f"Skipping question due to no retrieved documents: {question}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate markdown files for document relevance labeling"
    )
    parser.add_argument(
        "--site",
        type=str,
        required=True,
        help="Site ID to use for filtering documents and loading environment variables",
    )
    parser.add_argument(
        "--questions",
        type=str,
        required=True,
        help="Path to the file containing questions (one per line)",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
