...
import { getOrCreateUUID } from '../../src/utils/client/uuid';
import { updateLike } from '../../src/services/likeService';
...
// Mock dependencies
jest.mock('../../src/utils/client/uuid', () => ({
  getOrCreateUUID: jest.fn(),
}));

jest.mock('../../src/services/likeService', () => {
  let currentLikeCount = 5;
  return {
    updateLike: jest.fn(),
    getLikeCounts: jest.fn().mockImplementation((ids) => {
      return Promise.resolve({ [ids[0]]: currentLikeCount });
    }),
    _setMockLikeCount: (count: number) => {
...
jest.mock('../../src/utils/client/tokenManager', () => ({
  ...jest.requireActual('../../src/utils/client/tokenManager'),
...
describe('LikeButton', () => {
  ...
  it('toggles like state when clicked', async () => {
    ...
    const { _setMockLikeCount } = jest.requireMock(
      '../../src/services/likeService',
    );
    ...
    // Set mock to return 6 likes after clicking
    _setMockLikeCount(6);
    ...
    // Set mock to return 5 likes after unliking
    _setMockLikeCount(5);
