...
jest.mock('@/services/likeService', () => ({
  updateLike: jest.fn(),
  getLikeCounts: jest.fn().mockResolvedValue({}),
}));
...
describe('LikeButton', () => {
  ...
  it('toggles like state when clicked', async () => {
    // Mock getLikeCounts to return the expected value after update
    const { getLikeCounts } = jest.requireMock('@/services/likeService');
    (getLikeCounts as jest.Mock).mockResolvedValue({ [mockAnswerId]: 6 });
    ...
    await act(async () => {
      ...
      // Wait for async operations in handleLike to complete
      await Promise.resolve(); // Allow microtasks to run
    });

    // Wait for the callback which signifies async updates are done
    await waitFor(() => {
    ...
    (getLikeCounts as jest.Mock).mockResolvedValue({ [mockAnswerId]: 5 });
    mockOnLikeCountChange.mockClear();
