...
def ensure_environment_exists(environment_name, repo, dry_run=False):
    ...
    if dry_run:
        print(
            f"[DRY RUN] Would check if environment {environment_name} exists in repo {repo}"
        )
        return
        ...
        if key and value:
            if dry_run:
                ...
                try:
                    ...
                    print(
                        f"Error setting {key}: {e.stderr.decode() if e.stderr else str(e)}"
                    )
...
def upload_to_repo_level(env_vars, dry_run=False):
    ...
    repo = "anandaworldwide/mega-rag-chatbot"  # Hardcoded repository
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload environment variables to GitHub environments."
    )
    parser.add_argument(
        "--site",
        required=True,
        help="Short site name for environment file (e.g., ananda)",
    )
    parser.add_argument(
        "--repo-level",
        action="store_true",
        help="Use repository-level secrets instead of environment secrets",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be done without actually uploading secrets",
    )

    args = parser.parse_args()
    site = args.site
    repo = "anandaworldwide/mega-rag-chatbot"  # Always use this repository
    ...
    print(
        f"🔑 Uploading secrets for {site} from {env_file} to GitHub repository {repo}"
    )
