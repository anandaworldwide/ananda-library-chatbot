...
3. Delete by custom prefix:
   python delete_pinecone_data.py --site <site> --prefix "text||ananda.org||web||Some Title"
...
- Delete by custom prefix (new 7-part format):
  python delete_pinecone_data.py --site ananda --prefix "text||ananda.org||web||Page Title"
...
def construct_media_prefix(
    file_type: str, library: str | None = None, title: str | None = None
) -> str:
    prefix = f"{file_type}||"
    if library:
        prefix += f"{library}||"
...
def construct_source_prefix(
    source: str, type: str = "text", subsource: str | None = None
) -> str:
...
def delete_records_by_prefix(index, prefix: str) -> None:
    ...
    if confirmation in ["yes", "y"]:
        ...
        for i in tqdm(
            range(0, len(record_ids), batch_size),
            total=num_batches,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--site", type=str, required=True, help="Site ID for environment variables"
    )
    ...
    # Optional arguments
    parser.add_argument(
        "--library", type=str, help="Name of the library (only valid with --file-type)"
    )
    parser.add_argument(
        "--title",
        type=str,
    ...
    parser.add_argument(
        "--subsource", type=str, help="Subsource name (only valid with --source)"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    if args.file_type:
        ...
        if used_invalid_args:
            parser.error(
                f"When using --prefix, you cannot specify: {', '.join('--' + arg for arg in used_invalid_args)}"
            )
    ...
    if not PINECONE_API_KEY or not PINECONE_INDEX_NAME:
        raise ValueError(
            "PINECONE_API_KEY and PINECONE_INGEST_INDEX_NAME must be set in the environment or .env file."
        )
    ...
    if args.file_type:
        ...
        print("Deleting by custom prefix")
        prefix = args.prefix
