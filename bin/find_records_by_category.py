...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Find Pinecone records by category using metadata filter."
    )  # Updated description
    parser.add_argument(
        "--site",
        required=True,
    ...
    parser.add_argument(
        "--category", required=True, help="The category name to filter records by."
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=100,
        help="Max number of results to return from Pinecone query (default: 100). Max is 10000.",
    )  # Updated help text
...
def find_and_print_matches(index, category_name: str, dimension: int, top_k: int):
    ...
    print(
        f"Querying Pinecone index with filter for category '{category_name}' (top_k={top_k})..."
    )
    ...
    try:
        ...
        print(
            f"Pinecone query completed in {end_time - start_time:.2f} seconds. Received {returned_count} results matching the filter."
        )
...
def main():
    ...
    print(
        f"--- Finding Records for Category: '{args.category}' (Site: {args.site}) ---"
    )
    ...
    if args.top_k > 10000:
        print(
            "Warning: top_k exceeds the maximum recommended value (10000). Setting to 10000."
        )
        args.top_k = 10000
    elif args.top_k <= 0:
