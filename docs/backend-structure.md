...
- **`GET /api/audio/[filename]`** (`pages/api/audio/[filename].ts`)
  - **Purpose:** Serves audio files referenced as sources in chat responses.
  - **Auth:** Requires JWT authentication.
  - **Logic:** Streams file from AWS S3 based on the filename derived from source metadata.
